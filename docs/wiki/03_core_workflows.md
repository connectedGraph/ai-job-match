...
## 2. 学生简历解析流水线 (Resume Parsing Pipeline)

学生端支持上传简历图片，经由 OCR + LLM 级联提取生成结构化学生画像。核心实现位于 `career-planner/backend/app.py` 的 `POST /api/ai/resume/parse` 端点。

### 2.1 整体流程

```
[ 上传简历图片 (dataUrl: base64) ]
         │
         ▼
┌─────────────────────────────────────────────┐
│  Step 1: OCR 文字识别                        │
│  - 调用视觉模型（如 GLM-4V）对图片做 OCR     │
│  - 输出：纯文本简历内容（保留排版）            │
└─────────────────────────────────────────────┘
         │
         ├── OCR 成功 ──────────────────────────┐
         ...
         ▼                                      ▼
┌──────────────────────┐          ┌──────────────────────────────────┐
│ Step 2a: 错时并发提取  │          │ Step 2b: Vision 降级模式           │
│ (Staggered Launch)   │          │ (OCR 失败时直接用图片并发提取)      │
│                      │          │                                  │
│ 1. 先发: Experiences │          │ 三路并发:                          │
│    (最耗时)           │          │  - Experiences 提取               │
│                      │          │  - BasicInfo 提取                 │
│ 2. sleep(1.0s)       │          │  - Skills 提取                    │
│    让前缀进入缓存     │          │                                  │
│                      │          │ 均以 image_url 方式直接传图片       │
│ 3. 并发: BasicInfo   │          └──────────────────────────────────┘
│         + Skills     │
│    (命中前缀缓存)     │
└──────────────────────┘
...
- 调用配置中指定的视觉模型（通过 `load_ocr_config()` 加载，走 OpenAI 兼容的 `/v1/chat/completions` 接口）
- Prompt：`"请对图片中的简历内容进行完整的OCR文字识别，保留排版。"`
- 超时和 base_url 独立于旗舰模型配置
- 返回纯文本后作为后续 LLM 提取的输入

### 2.3 错时并发策略 (Staggered Launch)

这是本系统的一个性能优化设计：

1. **先发起 Experiences 提取**（通常最耗时，因为经历描述最长）
2. **等待 1 秒**：此时 LLM 提供商已将 System Prompt + OCR 文本的前缀写入 KV Cache
3. **再并发发起 BasicInfo 和 Skills 提取**：这两个请求共享相同的前缀（System Prompt + OCR 文本），大概率命中提供商的 Prompt Cache，显著降低首 Token 延迟

这个策略在使用支持 Prompt Caching 的模型（如 Claude、GPT）时效果最佳，可将三路总耗时压缩到接近单路耗时。

### 2.4 Vision 降级模式

当 OCR 步骤失败（网络超时、模型不可用等）时，系统自动降级为 Vision 模式：

- 直接将原始图片 base64 以 `image_url` 格式传入 LLM
- 三路并发提取（无错时优化，因为没有共享文本前缀）
- 对模型的多模态能力有更高要求

### 2.5 标签对齐 (Tag Alignment)

提取完成后，`tag_aligner.py` 负责将 LLM 输出的自由文本标签对齐到系统标准词表：

- 技术栈标签映射到 Tag Center 中的英文标准词
- 技术能力标签匹配已有的 normalizedTag
- 未匹配的标签保留原文，后续可通过管理后台归一化

---

## 3. 标签对齐治理与归一化 (Tag Governance & Normalization)
...
## 4. 确定性人岗匹配算法 (Deterministic Matching Engine)
